...
|   vxlapi.h
| Project:
|   Multi Bus driver for Windows 10 / Windows 11
|
| Description:
...
#define _XL_EXPORT_DECLSPEC_DLL __declspec(dllexport)
...
#define _XL_EXPORT_DECL _XL_EXPORT_DECLSPEC_DLL _XL_EXPORT_API
...
#define XL_HWTYPE_VTSSERVICE             110
#define XL_HWTYPE_VN1530                 112
#define XL_HWTYPE_VN1531                 113
#define XL_HWTYPE_VX1161A                114
#define XL_HWTYPE_VX1161B                115
#define XL_HWTYPE_VN1670                 120
#define XL_HWTYPE_VN5620A                121
#define XL_MAX_HWTYPE                    121
...
typedef XLuint64 XLindex;

#define XL_USE_ALL_CHANNELS      (XLuint64)0xFFFFFFFFFFFFFFFF
#define XL_INVALID_CHANNEL_INDEX (unsigned int)0xFFFFFFFF
#define XL_INVALID_DEVICE_INDEX  (unsigned int)0xFFFFFFFF
...
#define XL_SET_TIMESYNC_NO_CHANGE (XLulong)0
...
// flags for channelCapabilities - XLchannelDrvConfig
#define XL_CHANNEL_FLAG_EX_MASK(n)                       (1UI64 << n)
// Time-Sync
#define XL_CHANNEL_FLAG_EX1_TIME_SYNC_RUNNING            XL_CHANNEL_FLAG_EX_MASK(0)
#define XL_CHANNEL_FLAG_EX1_HWSYNC_SUPPORT               XL_CHANNEL_FLAG_EX_MASK(4)
#define XL_CHANNEL_FLAG_EX1_CANFD_ISO_SUPPORT            XL_CHANNEL_FLAG_EX_MASK(10)
// used to distinguish between VN2600 (w/o SPDIF) and VN2610 (with S/PDIF)
#define XL_CHANNEL_FLAG_EX1_SPDIF_CAPABLE                XL_CHANNEL_FLAG_EX_MASK(20)
...
#define XL_ETH_MODE_SPEED_FIXED_10       7 /* Set connection speed to 10 Mbps. Auto-negotiation disabled. */
...
////////////////////////////////////////////////////////////////////////////////
// Ethernet Network-based mode declarations
////////////////////////////////////////////////////////////////////////////////
...
#define XL_INVALID_NETWORKHANDLE (-1)
typedef XLlong XLnetworkHandle, *pXLnetworkHandle;
#define XL_INVALID_ETHPORTHANDLE (-1)
typedef XLlong XLethPortHandle, *pXLethPortHandle;
#define XL_INVALID_RXHANDLE (-1)
typedef XLlong XLrxHandle, *pXLrxHandle;

// Defines for XLnetworkDrvConfig::statusCode
#define XL_NET_CFG_STAT_OK                0x00
#define XL_NET_CFG_DUPLICATE_SEGMENT_NAME 0x01
#define XL_NET_CFG_DUPLICATE_VP_NAME      0x02
...
// TX events
typedef T_XL_ETH_DATAFRAME_TX T_XL_NET_ETH_DATAFRAME_TX;
...
typedef struct s_xl_device_drv_config_v1 {
  ...
  unsigned int hwType;   //!< HWTYPE_xxxx (see above)
...
*
*/
DECL_STDXL_FUNC(xlDestroyDriverConfig, XLDESTROYDRIVERCONFIG, (XLdrvConfigHandle configHandle));
 ...
 *  \return index of the selected channel or -1 if not found.
 */
DECL_STDXL_FUNC_T(int, xlGetChannelIndex, XLGETCHANNELINDEX, (int hwType, int hwIndex, int hwChannel));

/** \brief Get the channel mask for a channel of a certain hardware.
 *  \param  hwType                  [IN] hwType of the device or -1 for don't care
 *  \param  hwIndex                 [IN] hwIndex of the device or -1 for don't care
 *  \param  hwChannel               [IN] index of the channel on the device or -1 for don't care
 *  \return mask of the selected channel or 0 if not found.
 */
DECL_STDXL_FUNC_T(XLaccess, xlGetChannelMask, XLGETCHANNELMASK, (int hwType, int hwIndex, int hwChannel));
...
xlCreatePort():
--------------------------------------------------------------------------------
Create a new container (port) for application channels.
All channels of a port share the same receive queue.
The port handle is returned.
*/
...
xlAddChannelToPort():
--------------------------------------------------------------------------------
Adds a channel specified by the index to an application port.
The application tells the driver if it wants to get permission
...
xlFinalizePort():
--------------------------------------------------------------------------------
The application tells the driver to finalize the port initialization.
Afterwards no further channels can be added to the port.
*/

DECL_STDXL_FUNC(xlFinalizePort, XLFINALIZEPORT, (XLportHandle portHandle));
...
DECL_STDXL_FUNC(xlSetTimerRate, XLSETTIMERRATE, (XLportHandle portHandle, XLulong timerRate));
...
the function.
Applications set timerChannelMask to XL_USE_ALL_CHANNELS when the port has been created with xlCreatePort.
...
Utility Functions
*/
DECL_STDXL_FUNC_T(XLstringType, xlGetErrorString, XLGETERRORSTRING, (XLstatus err));
DECL_STDXL_FUNC_T(XLstringType, xlGetEventString, XLGETEVENTSTRING, (XLevent * pEv));
DECL_STDXL_FUNC_T(XLstringType, xlCanGetEventString, XLCANGETEVENTSTRING, (XLcanRxEvent * pEv));
...
DECL_STDXL_FUNC(xlGenerateSyncPulse, XLGENERATESYNCPULSE, (XLportHandle portHandle, XLaccess accessMask));

/*------------------------------------------------------------------------------
 ** xlPopupHwConfig():
 *\brief launch the Vector Hardware Manager (VHM) or Vector Hardware Config and optionally wait for it to finish.
 ...
 *
 * Starting with version 22.20, xlPopupHwConfig() prefers to call the Vector Hardware Manager (VHM).
 * It will fall back to the legacy Vector Hradware Config (VHwConfig, "vcanconf.exe") if
 * - a non-null and non-empty callSign is passed.
 * - the VHM is not installed.
 *
 * Note that independently of the waitForFinish value, xlPopupHwConfig() will not force the termination of VHM/VHwConfig
 * before returning. Instead VHwConfig continues to run as a child process.
 *
 ...
 *           XL_PENDING           if waitForFinish>0 and VHwConfig was successfully launched but has not exited after waitForFinish
 *milliseconds. XL_ERR_WRONG_COMMAND if waitForFinish>0 and VHwConfig exited with a non-zero exitcode.
 * --------------------------------------------------------------------------------
 */
...
DECL_STDXL_FUNC(xlDeactivateChannel, XLDEACTIVATECHANNEL, (XLportHandle portHandle, XLaccess accessMask));
...
DECL_STDXL_FUNC(xlClosePort, XLCLOSEPORT, (XLportHandle portHandle));
...
DECL_STDXL_FUNC(xlCanFlushTransmitQueue, XLCANFLUSHTRANSMITQUEUE, (XLportHandle portHandle, XLaccess accessMask));
...
DECL_STDXL_FUNC(xlCanSetChannelOutput, XLCANSETCHANNELOUTPUT, (XLportHandle portHandle, XLaccess accessMask, int mode));
...
DECL_STDXL_FUNC(xlCanSetChannelMode, XLCANSETCHANNELMODE, (XLportHandle portHandle, XLaccess accessMask, int tx, int txrq));
...
DECL_STDXL_FUNC(xlCanSetChannelBitrate, XLCANSETCHANNELBITRATE, (XLportHandle portHandle, XLaccess accessMask, XLulong bitrate));
...
DECL_STDXL_FUNC(xlCanSetChannelAcceptance, XLCANSETCHANNELACCEPTANCE,
                (XLportHandle portHandle, XLaccess accessMask, XLulong code, XLulong mask, unsigned int idRange));
...
DECL_STDXL_FUNC(xlCanAddAcceptanceRange, XLCANADDACCEPTANCERANGE,
                (XLportHandle portHandle, XLaccess accessMask, XLulong first_id, XLulong last_id));
DECL_STDXL_FUNC(xlCanRemoveAcceptanceRange, XLCANREMOVEACCEPTANCERANGE,
                (XLportHandle portHandle, XLaccess accessMask, XLulong first_id, XLulong last_id));
DECL_STDXL_FUNC(xlCanResetAcceptance, XLCANRESETACCEPTANCE, (XLportHandle portHandle, XLaccess accessMask, unsigned int idRange));
...
DECL_STDXL_FUNC(xlCanRequestChipState, XLCANREQUESTCHIPSTATE, (XLportHandle portHandle, XLaccess accessMask));
...
DECL_STDXL_FUNC(xlCheckLicense, XLCHECKLICENSE, (XLportHandle portHandle, XLaccess accessMask, XLulong protectionCode));
...
DECL_STDXL_FUNC(xlLinSetSlave, XLLINSETSLAVE,
                (XLportHandle portHandle, XLaccess accessMask, unsigned char linId, unsigned char data[8], unsigned char dlc,
                 unsigned short checksum));
DECL_STDXL_FUNC(xlLinSendRequest, XLLINSENDREQUEST,
                (XLportHandle portHandle, XLaccess accessMask, unsigned char linId, unsigned int flags));
DECL_STDXL_FUNC(xlLinSetSleepMode, XLLINSETSLEEPMODE,
...
DECL_STDXL_FUNC(xlLinSwitchSlave, XLLINSWITCHSLAVE,
...
DECL_STDXL_FUNC(xlDAIOSetPWMOutput, XLDAIOSETPWMOUTPUT,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int frequency, unsigned int value));
DECL_STDXL_FUNC(xlDAIOSetDigitalOutput, XLDAIOSETDIGITALOUTPUT,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int outputMask, unsigned int valuePattern));
DECL_STDXL_FUNC(xlDAIOSetAnalogOutput, XLDAIOSETANALOGOUTPUT,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int analogLine1, unsigned int analogLine2,
                 unsigned int analogLine3, unsigned int analogLine4));
DECL_STDXL_FUNC(xlDAIORequestMeasurement, XLDAIOREQUESTMEASUREMENT, (XLportHandle portHandle, XLaccess accessMask));
DECL_STDXL_FUNC(xlDAIOSetDigitalParameters, XLDAIOSETDIGITALPARAMETERS,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int inputMask, unsigned int outputMask));
DECL_STDXL_FUNC(xlDAIOSetAnalogParameters, XLDAIOSETANALOGPARAMETERS,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int inputMask, unsigned int outputMask,
                 unsigned int highRangeMask));
DECL_STDXL_FUNC(xlDAIOSetAnalogTrigger, XLDAIOSETANALOGTRIGGER,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int triggerMask, unsigned int triggerLevel,
                 unsigned int triggerEventMode));
DECL_STDXL_FUNC(xlDAIOSetMeasurementFrequency, XLDAIOSETMEASUREMENTFREQUENCY,
                (XLportHandle portHandle, XLaccess accessMask, unsigned int measurementInterval));
DECL_STDXL_FUNC(xlDAIOSetDigitalTrigger, XLDAIOSETDIGITALTRIGGER,
...
DECL_STDXL_FUNC(xlKlineSwitchTesterResistor, XLKLINESWITCHTESTERRESISTOR, (DEFFRPARAM, unsigned int testerR));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostWriteRegister, XLFP_MOSTWRITEREGISTER,
                (DEFPARAMS, unsigned short adr, unsigned char numBytes, unsigned char data[16]));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostWriteRegisterBit, XLFP_MOSTWRITEREGISTERBIT,
                (DEFPARAMS, unsigned short adr, unsigned char mask, unsigned char value));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostCtrlSyncAudioEx, XLFP_MOSTCTRLSYNCAUDIOEX,
                (DEFPARAMS, unsigned int channel[16], unsigned int device, unsigned int mode));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostGenerateLightError, XLFP_MOSTGENERATELIGHTERROR,
                (DEFPARAMS, XLulong lightOffTime, XLulong lightOnTime, unsigned short repeat));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostGenerateLockError, XLFP_MOSTGENERATELOCKERROR,
                (DEFPARAMS, XLulong unmodTime, XLulong modTime, unsigned short repeat));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMostCtrlGenerateBusload, XLFP_MOSTCTRLGENERATEBUSLOAD, (DEFPARAMS, XLulong numberCtrlFrames));
...
DECL_STDXL_FUNC(xlFrSetupSymbolWindow, XLFP_FRSETUPSYMBOLWINDOW, (DEFFRPARAM, unsigned int frChannel, unsigned int symbolWindowMask));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150GenerateLightError, XLFP_MOST150GENLIGHTERROR,
                (DEFPARAMS, unsigned int lightOffTime, unsigned int lightOnTime, unsigned int repeat));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150GenerateLockError, XLFP_MOST150GENLOCKERROR,
                (DEFPARAMS, unsigned int unlockTime, unsigned int lockTime, unsigned int repeat));
 ...
 *  \return XLstatus                      general status information
 */
DECL_STDXL_FUNC(xlMost150CtrlGenerateBusload, XLFP_MOST150CTRLGENLOAD, (DEFPARAMS, XLulong numberCtrlFrames));
 ...
 *  \return XLstatus                       general status information
 */
DECL_STDXL_FUNC(xlMost150GenerateBypassStress, XLFP_MOST150GENERATEBYPASSSTRESS,
                (DEFPARAMS, unsigned int bypassCloseTime, unsigned int bypassOpenTime, unsigned int repeat));
 ...
 *  \return XLstatus     general status information
 */
DECL_STDXL_FUNC(xlNetCloseNetwork, XLFP_NETCLOSENETWORK, (XLnetworkHandle networkHandle));
 ...
 *  \return XLstatus     general status information
 *
 * The receive queue is implicitly flushed by this function.
 */
DECL_STDXL_FUNC(xlNetActivateNetwork, XLFP_NETACTIVATENETWORK, (XLnetworkHandle networkHandle));
 ...
 *  \return XLstatus     general status information
 */
DECL_STDXL_FUNC(xlNetDeactivateNetwork, XLFP_NETDEACTIVATENETWORK, (XLnetworkHandle networkHandle));
 ...
 *  \return XLstatus           General status information
 */
DECL_STDXL_FUNC(xlNetEthRequestChannelStatus, XLFP_NETETHREQUESTCHANNELSTATUS, (XLnetworkHandle networkHandle));
 ...
 *  \return XLstatus           general status information
 */
DECL_STDXL_FUNC(xlNetFlushReceiveQueue, XLFP_NETFLUSHRECEIVEQUEUE, (XLnetworkHandle networkHandle));
////////////////////////////////////////////////////////////////////////////////
