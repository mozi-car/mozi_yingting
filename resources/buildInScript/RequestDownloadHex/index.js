...
// Queued blocks parsed from the hex file
const pendingBlocks = [];
...
Util.Init(() => {
  ...
  // Register main RequestDownloadHex function
  Util.Register(`${testerName}.RequestDownloadHex`, async function(dataFormatIdentifier, addressAndLengthFormatIdentifier, hexFilePath) {
    ...
    // parse hex file into discrete memory blocks
    const hexText = await fsP.readFile(hexFilePath, 'utf8');
    const memMap = ECB.HexMemoryMap.fromHex(hexText);
    for (const [addr, data] of memMap) {
      pendingBlocks.push({ addr, data });
    }

    if (pendingBlocks.length === 0) {
      throw new Error("hex file contains no data");
    }
    ...
    const job = new ECB.DiagJob(testerName, {
      id: "RequestDownloadHex_tmpJob",
      name: "RequestDownloadHex_tmpJob",
      serviceId: "Job",
      params: [],
      respParams: []
    });
  ...
  Util.Register(`${testerName}.RequestDownloadHex_tmpJob`, () => {
    // Validate chunk size
